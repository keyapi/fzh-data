...
## 2026-09-23 核查（87 张未付）

整月口径下未付 87 张，改用 `ups_track` 模块 + UPS 官方 Track API **一次批量查完**（不再浏览器逐单）。

| 结果 | 张数 | 说明 |
|------|------|------|
| 已交付 | 86 | 已实际发货并签收，PB 付款约在发货后 30 天；多张落在下个账期 |
| 未交运 | 1 | INV0580626000011890 |

**INV0580626000011890**（PO 137682252，跟踪 1ZC0019E0337531455）：UPS 只到
*"Shipper created a label, UPS has not received the package yet."*（标签 08/14 创建，之后 UPS 从未收件）。
经与业务确认：**该单无货未发**，11 月中才到货、不一定继续发；钉钉 8 月底已提交，暂不处理，只在 Notes 写文字备注。

**其余 86 张** 全部有实际发货时间（08/25 起），交付日期正常，属"还没到 PB 付款日"而非漏结算。
典型：20260824 那批 SPS 日期 08/24、实际 08/25 发货 → PB 约 09/24 付款，正好错过本次 09/23 检查日。

**结论**：本轮没有"PB 漏结算"的案子；唯一异常是 1 张无货未发（我方原因，非 PB）。
