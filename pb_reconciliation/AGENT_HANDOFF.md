...
- 发票范围 = [首个有付款的发票日, 最后一个]（**含整天无付款日**）；未付清单含范围内未付发票。
- 上轮未付本轮已付：读上一账期 TM 文件（`PREV_SOURCE`）或自动衔接上一期。
- **硬校验**：付款总额须与财务确认一致（当前账期：05/19-06/18 = $14,185.71，06/19-07/18 = $8,842.75）。
- 2026-08-14 已生成：`PB Remittance Advice Payment Date 20260519-20260618.xlsx`（佣金 $709.29）、`...20260619-20260718.xlsx`（佣金 $442.14）。
