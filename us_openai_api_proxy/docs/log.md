...
- **v0.14-fix**: rueducommerce 直连规则 — Mirakl 法国后台 `groupe-rueducommerce.fr` 在 BoostNet 日本/美国节点被该站 Cloudflare TLS 重置，仅香港能通但自定义规则无法引用脚本创建的 `香港` 组；直连实测 6/6 全通且快速，加 `DOMAIN-SUFFIX,groupe-rueducommerce.fr,DIRECT`
