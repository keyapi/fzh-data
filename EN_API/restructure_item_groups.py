...
流程:
  ...
  2. 获取 EN 测试系统当前 Item Group
  3. 创建赛狐分类节点（如不存在）
  4. 将 EN 产品节点移动到对应赛狐分类下
  5. 验证

使用:
  python restructure_item_groups.py --dry-run    # 预览
  python restructure_item_groups.py              # 执行
"""
...
_ENV_URLS = {"test": "https://ensh.vilavi.cn"}
...
def build_moves(
    ...
    # 反向映射: EN name 是 custom_model_id 或 item_group_name
    # 先用 custom_model_id 匹配
    for en_name, en_node in idx.items():
        ...
        # 若目标分类名与产品自身名称相同（如产品"三角靠枕"本身就是
        # 赛狐"三角靠枕"分类），则上移一级至父分类，避免自引用冲突。
...
def en_node_exists(name: str, parent: str, en_data: list[dict]) -> bool:
    """检查 EN 系统中是否存在 (item_group_name, parent_item_group) 组合。"""
...
def en_name_exists(name: str, en_data: list[dict]) -> bool:
    """检查 EN 系统中是否存在同名节点（by item_group_name）。"""
    name = name.strip()
    for d in en_data:
    ...
    for m in moves:
        ...
        if not dry_run:
            try:
                ...
                # 若因节点有子级导致无法转移（不能是叶节点），先设 is_group=1 再重试
...
def verify(
    ...
    rpt = out_dir / f"物料组重构验证_{ts}.xlsx"
...
def en_name_exists(name: str, data: list[dict]) -> bool:
    name = name.strip()
    for d in data:
        if d.get("item_group_name") == name:
            return True
...
def _get_error_body(exc: requests.RequestException) -> str:
    """Extract decoded error message from API error response."""
...
def write_op_report(
    ...
    path = out_dir / f"物料组重构操作{tag}_{ts}.xlsx"
...
def main() -> int:
    import argparse
    ap = argparse.ArgumentParser(description="按赛狐分类重构 EN 测试系统物料组")
    ...
    client = ErpnextClient(_ENV_URLS["test"], test_key, test_secret)
    print(f"测试环境: {_ENV_URLS['test']}")
    ...
    # 列举未匹配的 EN 产品（有 custom_model_id 但不在赛狐映射中的）
    idx = build_index(en_data)
    unmatched = []
