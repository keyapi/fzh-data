...
6. **单价/总价统一 = BOM 成本 × 加成**：**4 个 sheet 全部**用 BOM 成本，即单价 = `bom_rate` ÷ 6.8 × 1.45、总价 = `bom_cost` ÷ 6.8 × 1.45、TOTAL = `Σbom_cost` ÷ 6.8 × 1.45（`usd_price()` 封装）。汇率暂定 6.8、加成系数 `price_markup=1.45`（均暂定，后续换汇率表/确认加成）。
