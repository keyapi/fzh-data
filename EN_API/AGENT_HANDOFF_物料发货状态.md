...
| Sheet | 内容 | 行粒度 | 列数 |
|-------|------|--------|------|
| 订单发货状态 | 订单行 + 出库/工单/工序/生产计划汇总 | 一行一 (SO × 订单行) | 28 |
| 出库明细 | 出库行明细 + 合计行 | 一行一 (SO × DN × 行) | 18 |
| 工单与工序 | 每张工单的每道工序 + 工序卡 | 一行一 (WO × 工序) | 15 |

Sheet1 第 2 列是**内部流转号**（EN `po_no`/`purchase_order`，**非客户 PO，待废弃**）。
控制台的「订单发货状态」与「交付预估」两张表也各有一列 —— 交付预估那张是给物流/计划看的，
旁边那行提示写明了头程要走 EN 销售订单编号查钉钉表。
...
8. **EN 的 `po_no` 不是客户 PO 号** —— 2026-09-15 经计划物流同事核实：它是**离职同事自编的
   内部流转号**，不是客户给的采购订单号。当年用途是「一张 SO 对应多个客户 PO 时，
   用来区分下单日期 / 仓库」。现在 SO 自身已有下单日期和仓库，所以**意义有限、之后可能弃用**。
   脚本仍输出它（过渡期仍能区分同单不同批次），但列名标成
   **「内部流转号(非客户PO,待废弃)」**。
   - **不得**对外称客户 PO；**不得**拿它查头程。
   - 头程在**钉钉表**里，用 **EN 销售订单编号**（`Sales Order.name`）匹配 —— 见
     `dingtalk/dingtalk_sheet/AGENT_HANDOFF.md`。
...
fixture 只对工序做**下界**断言（`⊇ 前 4 道`，工序只增不减）；工序的当前状态归「在产快照」档，
只报漂移、不影响退出码。**不要**对在产工序做上界断言（比如"质检尚未完成"）—— 当天 17:06
质检就完工了，那种断言会在几小时内自己变红。
...
- **`--assert-fixture` 只认 `--customer-code CENKZ1325-Yellow-138`**：`--item` 是不限客户的超集
  （本例 18 张 vs 10 张），对不上 fixture 的 `so_count`。断言分两档：
  - **稳定档**（订单结构 `so_count`/`order_qty`、历史出库行数量、`master_item`、预估可发推导、
    工序下界）→ 失败即 exit 1，这些正常经营下不会变，变了就是回归。
  - **在产档**（`open_qty`/`in_progress_qty`/`dead_sos`/`wo_count`/工序完成件数/剩余工序），
    基线取 2026-09-14 → 只打印「漂移」，不影响退出码。

  之所以拆开：这是个**活业务对象**，本工具监控的"发货"一旦成功，在产档的数必然变 ——
  当断言用会让工具**因为它要监控的事做成了而变红**。漂移了先核实数据，别为了变绿直接改数字。
  list 查询 HTTP ≠ 200 会 `QueryError` 退出，不当空结果继续算。
...
2. **`Work Order.status` 不可信** —— `WO-26-02609` 写 `Not Started`，实际 44 件已过裁剪等前道工序
   （基线 2026-09-14 为 4 道；同一天 14:22 翻面、17:06 检查皮壳扭筋 与 质检 相继完工，
   现在 `WO-26-02609` 已是 7/9 道、头部 `status=Completed`、`produced_qty=44`，
   只剩 通用返工 / 质检发现问题 两个返工分支 Pending）。
