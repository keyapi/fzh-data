...
description: >
  ...
  不用于只计算采购成本、商品重尺。
  不把 PK#/HM1510 建成赛狐**有库存普通商品**；三角皮壳 Listing 的共享库存代理走 sellfox-cover-inventory。
---
...
- 赛狐三方主线对象仍是 EN 产品 `item_code`（`KS` 普通商品）。不要用通途 `-Cover/-Foam` 原码、`HM1510` 或把 `PK#` 建成**有库存普通商品**。三角类确有皮壳 Listing 时的 `PK# -> KS x1` 组合代理不走本流程，见 `sellfox-cover-inventory`。
