...
铁律：缓存用 `fileId__原名`；对照 DRM「提交窗/人名」vs 核算「账期月」；负责人看渠道账号表 `运营人员YYYYMM`，不看 NAS 人名夹；赛狐店名经别名对 `AMZ*` 文件前缀；不改 `D:\NAS与我共享\`；真搬文件只走 NAS API。不下已撤销/拒绝、不下图片。Amazon 账期只认 `.txt`。离职账号附件 API 常 `userNotExist`（单上可能仍有文件）。
