...
| 谁能用 | 有命令行环境的人 | 有钉钉账号的人（公网实例） |

网页版把每次出件都记进任务库：任务编号、操作者、耗时、代码版本、输入文件（原始文件名）、
每个产物的 SHA-256 都能追溯。

> ⚠️ **当前限制**：登录用的是公司钉钉 OIDC 桥，而桥的 corpId 校验实际不生效
> （请求没申请 `corpid` 权限，见 `../docs/solutions/integration-issues/dingtalk-sso-new-api-oidc-bridge.md`）。
> 所以**任何钉钉账号**都能登录，不只是本公司员工。要做真正的公司内限制，
> 得改桥的授权 scope（影响所有走这条桥的服务），或填 `PB_ORDERS_ALLOWED_USERS` 白名单。
