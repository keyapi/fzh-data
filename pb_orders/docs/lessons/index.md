...
## L9. 报告时间戳之类的"每次都不同的字段"，要先归一化再比

标签 PDF 里每页盖了一个运行时间戳，所以新旧产物的像素差异**永远不为 0**，
一开始看到 0.4%–0.7% 的差异就"解释成时间戳"收工了。

正确做法：**把变化字段归一化后重建再做 diff** ——
把时间戳强制成老产物里的那个值重建一次，再逐页 diff：

```
用相同时间戳重建后：100 页总不同像素 = 0
```

这一步把"大概是时间戳吧"变成了"除了时间戳，一个像素都不差"，也就顺带证明了
把时间戳改到别处（当时那个 bug）不会被这句话掩盖过去。

## L10. 单位翻译错误：`36 * mm + 30` 里的 30 是 point，不是 mm

迁移叠加页坐标时，我把 notebook 的 `drawString(x5 + 30, y5 + 140)` 读成
"x5 是 36mm，加 30 → 66mm"，于是写成 `TS_1 = (66 * mm, 215 * mm)`。

但 reportlab 的单位本来就是 point：`36 * mm` 已经算成了 102.05，`+ 30` 加的是 **30 point**。
正确值是 `(102.05 + 30, 212.60 + 140) = (132.05, 352.60)`。

后果很隐蔽：叠加元素会按裁切框分区落到两页中的**某一页**，
坐标错位后两个时间戳都掉进打包单区域，**标签页直接没有时间戳**（用户一眼看出来了，
而我的自动 diff 只报"有差异"，没告诉我是"少了东西"）。

**教训**：
- 迁移坐标不要把"算式"提前算成常量再手写 —— 常量区直接照抄 notebook 的变量名与算式
  （`X5 + 30`），让单位由 reportlab 自己决定，人不去做单位换算。
- **分区输出要每页都看**：同一个叠加层会被裁成两页，改一处坐标必须两页都验证。
