...
- **做法**：建任务时把上传的输入也登记成产物（复用既有的 `input_packslip` /
  `input_order` 两个 kind 与中文名），走内容寻址存储 → **下载入口就放在任务页顶部
  「输入」那一行**（使用者原话：「人自然会去操作上传的上面找，不需要劈开成 2 部分放在下面」），
  每个输入旁边给「下载」+ SHA-256。
  ⚠️ 第一版把入口放在页面下方的独立区块里，页面有十几个区块、它排第 10 个，
  使用者**根本没发现** —— 功能在但要人往下翻就等于没有。
- **摆放踩了两轮（记下来）**：第一版放在页面下方独立区块 → 使用者说看不到；
  第二版改成顶部行内一行一个 `类型：文件名 [下载] hash` → 使用者说「超过 2 个按钮的位置没对齐，能不能变成列表」。
  最终：**顶部「输入」行里放一张表**（类型/文件名/大小/SHA-256/下载），
  列对齐、和「产物下载」同一套样式。教训：**同一区域的多个同类条目要用表格，
  不要用行内拼接** —— 文件名长短不一会让按钮位置参差。
- **关键约束**：新增 `storage.publish_input_copy()` —— 与 `publish_artifact` 不同，
  **绝不移走原文件**（inputs/ 里那份还要给 worker 用、还要供「用相同输入重新处理」）。
  优先**硬链接**（inputs/ 与 artifacts/ 同在 /runtime 同一卷，不额外占磁盘），
  不行再复制。已加断言：登记后 `inputs/<job>/order.csv` 必须还在。
- **覆盖 4 条建任务路径**：库存预检、直接出件、续出件（显示被复用的 checked CSV）、重跑。
- **登记失败不连累建任务**（吞异常并记日志）：少一个下载口也比建不出任务强。
- **测试 144 → 147**：输入可下载且内容逐字节等于上传件、输入/产物分区、
  续出件任务能看到复用的 checked CSV；另更新 3 处断言
  （仅校验 /「校验失败」这类语义改成"无**产物**，输入仍在"）。

## 2026-09-24（第十五轮：修「数据行末尾多空字段」读不进来 —— 真实批次踩到）

- **现象**：用户上传 `已和2单手动合并 check0stock order x16 20260924_0145_20820.csv`
  （16 单，由 14 单主导出 + 2 个单独 PO 导出手工合并）报 `csv_read_failed`，
  页面只给一句「无法读取 SPS 订单 CSV / 请确认没被 Excel 另存过」。
- **真因**：20260924 批次的形状是 **表头 147 字段 / H 行 146 / D 行 148** ——
  D 行在所有列之后多**一个空字段**（SPS 自己导出就这样）。pandas 遇到「比表头多」的行
  直接 `ParserError: Expected 147 fields in line 3, saw 148`，整批读不进来。
  **不是手工合并弄坏的**：逐字节比对了 3 个源文件与合并结果（32 行进、32 行出、
  无重无漏、逐行一致），并且**未经手改的原始导出同样失败**。
- **修法**：新增 `pb_tongtu_excel.strip_trailing_empty_fields()` ——
  多出来的**都是空的**就削掉（在所有列之后，不影响列对齐）；**有非空值就拒绝**
  （无法判断属于哪一列）。两个读取器共用：`stock_precheck._read_csv` 与
  `pb_tongtu_excel.load_order_csv`（出件阶段读 checked CSV，也会撞同一个坑）。
  checked CSV 仍**逐行照搬原文**（空字段一起照搬），字节契约对任何形状都成立。
- **不静默**：削掉空字段会在报告里留 `warnings`（页面新增「提醒」区块 + CLI `[提醒]`）。
- **错误信息不再吞真因**：带上 pandas 原文 + 可能原因 + 下一步；
  `invalid_record_type` 里的空值显示成 `(空)`（以前提示以冒号结尾、看着像 bug）。
- **安全网**：只看尾部空不空判断不出*中间*错位，靠 `_validate`
  （Record Type 必须 H/D + PO/Line/两列 SKU 非空 + 数量正整数）兜。
  实测末尾非空 → `ragged_source`；中间插入/删除 → `invalid_record_type`。
- **回归**：20260917 批次仍**字节级一致**（sha256 `e99a9b78…` 不变）。
  测试 138 → 144。
