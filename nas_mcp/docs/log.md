...
- **新增（按 mrquj + DSM API 盘点补充）**: 工具 11 → **13**：
  - **`nas_list_shares`** —— 列**该账号能看到的共享文件夹**（实测返回 `FZH共享文件夹` + `产品信息`）。
    价值：让模型**先知道有什么**，不用猜路径。
  - **`nas_folder_thumbnails`** —— **一次返回一个文件夹里的多张缩略图**。实测每张约 **10 KiB**，
    远低于读原图（每张 ~287 KiB）—— 6 张图能省约 1.7 MB。用于「这目录里都有什么图」。
- **盘点后决定「不做」的（附理由，免得反复讨论）**：
  - `list_archive`（看压缩包内容）—— **两个共享文件夹里都没有任何 zip/rar/7z**，没有真实文件可验证。
    **不发未测试的工具**。等真有压缩包再补（那时可验证）。
  - `check_permissions` —— 签名是 `(path, filename, overwrite)`，是**写权限检查**，与只读定位不符。
  - `favorites` / `background_task` —— 实测都返回**空**，没数据。
  - 分享链接（`Sharing`）—— 有用（给人发链接 / 大文件逃生口），但**会把文件暴露到公网**，
    属于改变外部状态，**等用户拍板**。
