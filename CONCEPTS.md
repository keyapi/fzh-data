...
### 内部流转号 (EN po_no)
EN 销售订单上的一个**内部自编字段**（`po_no` / 行级 `purchase_order`），
**不是客户给的采购订单号** —— 它由一位已离职的同事创建，用途是「一张 SO 对应多个客户 PO 时，
区分下单日期与仓库」。SO 自身已有下单日期和仓库，所以它意义有限、**待废弃**，
改由钉钉登记表按 EN 销售订单编号匹配。

因此它只能内部流转用：**不得对外称客户 PO，不得用它当跨系统查头程的键**。
*Avoid:* 客户 PO 号

### 头程 (First Leg)
工厂出库后、国外分公司仓签收前的这一段运输：待装柜 → 装柜 → 开船 → 海上 → 到港 → 清关提柜
→ 国外仓收货。它**不在 EN 里** —— EN 只到「工厂出库」为止，出库不等于到货。

这一段由计划和物流同事在**钉钉表**人工维护：一批货一个**批次号**，批次上记**离港 / 到港 /
入库**日期；明细里记到箱号。业务常退回「下单 + 若干月」的粗估，那是登记或匹配缺口的兜底，
不是比实际事件更优的口径 —— 一旦有离港、到港或入库事实，就应逐级替代它。

### 批次号 (Shipment Batch)
头程的主键。形如 `NJHY…`（美东）/ `TXHY…`（美中）/ `PL…`（波兰）/ `LEFBAUK…`（FBA），
由目的分公司前缀 + 日期构成。一个批次装多张销售订单的货，一张销售订单的行也可以拆到多个批次，
所以批次与销售订单行是**多对多**关系 —— 不能只在一张订单头上存一个批次字符串。
在明细表里批次号常常只写在批次首行（合并单元格），读取时要向下填充。

### 物流号 (ZMT / 货代单号)
货代或承运人给的运输单号，记在头程登记表的**物流单号**列（厂侧登记常见 `ZMT…` 前缀，
批次跟踪页可见 `HPL…`、`1ZB…` 等其他格式）。它是**承运人维度**的单号，
与批次号（我们自己编的发运批次）不是同一个东西：一批可能对应多个物流号，反之亦然。

### 工厂四件套 (Factory Handoff Columns)
首程登记表上代表「工厂侧是否已把头程该做的做完」的四列，按顺序为：
**工厂确认交期 → 包装完成日期 → 实际包装完成量 → 物流发票编号**。

这四列是**逐级推进**的：确认交期后才会有包装完成日期与完成量，包装完成后货才能交给物流并
拿到物流发票编号。所以「生产已完成但一直没发」的原因，看这四列**停在哪一级**就能定位 ——
比任何状态字段都直接。

### 国外仓签收/上架 (Overseas Receipt and Shelving)
货物到达国外分公司仓并被签收、清点、必要时二次加工后转为可售的最终事实
（登记表上体现为分公司的「签收日期 / 到货数量」，实际只有部分分公司登记了这两列）。
它是履约链的终点，也是唯一能证明「货真的到了」的证据。**任何上游影子数据都不能替代它**：
赛狐库存用安全冗余数量、海外仓备货单表达的是采购与成本口径，二者都不能反推某批头程已签收或已上架。
