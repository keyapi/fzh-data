...
## 通途历史尾程费率表 (EN history last-leg)

### 历史尾程费率模型 (History Last Leg Fee Record)
ERPNext 侧的发布式费率快照：parent 带模型状态/版本/适用期，child 逐行给出
「匹配维度 → 费率」。线上按由细到粗的层级链取首个命中。与旧记录 `HLF0001` 并存。

### HLF0001
迁移前的唯一历史尾程记录（2025-05-01~2025-10-31，3,037 子行），全国均价 + 最近重量匹配。
**全程不改不迁移**，作为最低优先级兼容兜底；其 `fg_unit_weight` 存的是**克**，
而线上兜底传的是**千克**，所以任何非精确匹配的包裹都会恒定命中该国最小重量记录
（美国恒返 ¥0.2638）。该缺陷在 legacy 适配器里按既有行为复刻，不修数据。

### 整包价 (per-package price) vs 单件价
承运商按**整包**计费：件数增加几乎不抬价（实测 1 件 119.78 / 2 件 124.94 / 3 件 111.58）。
因此历史尾程费率存的是整包价，`预估 × 发货数量` 是错的。见
`tongtool_order_cost/docs/solutions/logic-errors/history-last-leg-per-package-vs-quantity.md`。

### 月初占位值 (month-start placeholder)
月初导出上月订单时真实尾程尚未回传，工作流会先把 EN 的预估写进 `物流商运费` 占位，
之后由真实费用替换。含义：`物流商运费` 是唯一真值列，但**读历史月份时它可能仍是旧预估**——
把这种值当标签会形成循环训练。区分方法：与通途系统或后续导出逐包裹对账。

### 可信度加权 (credibility weighting) / 部分池化 (partial pooling)
细层样本不足时把小样本自身经验与粗层经验按权重混合：`C = Z·X + (1−Z)·M`，
`Z = n/(n+k)`，`k = 组内方差 / 组间方差`。业界（保险精算）标准做法。
本项目的实测结论是**增益≈0，保留实现但不启用**，取舍过程见
`tongtool_order_cost/docs/solutions/best-practices/rate-table-thin-cell-method-selection.md`。

### 层级契约 LEVEL_NAMES
分析侧（fzh-data）与线上侧（tongtool_integration）共享的 11 个层级名字面量，
必须逐字一致；两侧各有测试锁定，导出时出现线上不认识的层级会直接报错。
这是跨仓库契约，改一侧必须同时改另一侧。

### 排他发布 vs 独立发布 (exclusive / non-exclusive publishing)
排他：细层发布后把命中样本移出池子再发下一层——会让「薄维度」的细层把数据吃光，
导致粗层没数据、覆盖率归零。独立：各层都从全量统计，由解析器按细→粗取首个命中。
本项目定稿用**独立发布**。

### 月初工件 (month-start artifact)
月初导出的上月订单文件对（一份含 EN 预估的输出 + 一份事后补上真实费用的导出），
用于评估「预估填补」的真实效果。这是评估该模型唯一有意义的口径——
月均费率评估会掩盖月初缺失场景，而月初正是预估唯一真正被使用的时候。
