...
## 2026-09-15
- **整包价分摊验证脚本**: 新增 `scripts/verify_package_allocation.py`，用真实月初工件对比
  「老口径 ×件数 / 逐行写整包价 / 按整包分摊」三种口径的订单级合计，供独立复算
  （2026-05：+67.2% / +84.8% / **−2.4%**）。脚本不记录个人目录，工件路径由参数传入。
- **交接补充**: EN 侧调用方要求（按整包计费重量查一次 + 按行分摊）与实测对比。

## 2026-09-15
- **整包价语义锁定**: `test_observation_fee_is_package_total_not_per_unit` 锁定 `观察费用` 是整包总额（不按件数折单价）。
  说明：该性质**不能**用 `predict_with_variant` 验证（它不读件数，断言会永真）；调用方是否 × 件数由 EN 侧源码守卫测试锁定。
