...
两个时间窗 × 4 个样本门槛（月份门槛 1 与 2 结果**完全相同**，因为 ≥5 样本的层
天然跨 ≥2 月，故保留更保守的 2 月）：

| 门槛 | 层数 | 窗A MAE | 窗B MAE | 202605 | 202606 | 窗B 覆盖率% |
|------|------|------|------|------|------|------|
| 5/2 | 6,478 | 18.59 | 14.99 | 17.66 | 14.22 | 100.0 |
| **10/2** | **2,921** | 18.92 | 15.02 | **17.64** | **14.18** | 99.9 |
| 20/2 | 1,303 | 19.67 | 15.24 | 17.87 | 14.32 | 99.7 |
| 30/2（原基线） | 802 | 20.08 | 15.55 | 18.00 | 14.72 | 99.6 |

**10/2 与 5/2 精度实质相同**（窗A 相差 1.8%、窗B 相差 0.2%，两个月工件甚至略优），
但发布行数只有 **45%**。按「增加的复杂度是否划算」的判据，**推荐 10/2**。
（原推荐 5/1 已被本节取代。）
