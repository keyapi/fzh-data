...
| **`V3_低门槛5`**（V1b + 门槛降到 5/1） | **14.99** | **17.66** | **14.22** | **99.9%** | 6,500 |
| `V3b_低门槛5_可信度` | 14.97 | 17.66 | 14.21 | 99.9% | 6,500 |
...
3. **把样本门槛从 30/2 降到 5/1 再放开层级**：15.55 → 14.99（−4%），**覆盖率反升到 100%**。
   性价比很高，几乎不增加复杂度。
...
**建议采纳 `V3_低门槛5`**（结构修复 + 分区优先 + 5/1 门槛，不加可信度、不排平坦），
其余变体作为对照保留在同一工作簿中。
