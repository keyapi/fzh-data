...
2. **`Work Order.status` / `produced_qty` 不可信** —— `WO-26-02609` 头部写
   `Not Started`、`produced_qty = 0`，但 14 张工序卡显示 **44 件（2 批 × 22）** 已过
   `裁剪 / 皮壳整件 / 锁扣眼 / 拷边` 等 5 道工序。
...
- **完成件数取第一道工序（裁剪/开料）的完工量**。每道工序都有自己的工序卡，求和会把同一批
  件数按工序数重复累加 —— `WO-26-02609` 会算成 4×44=176，实际就是 **44**（2 批 × 22）。
...
| 异常 | 事实 | 含义 |
|------|------|------|
| 死单 | `SO-26-00099` Closed、40 件未发、交货日 2026-08-30、**无工单无生产计划** | ERP 已关闭，实际不会发 |
