...
### 10. EN 的 `po_no` 不是客户 PO 号 —— 头程要用 SO 编号查钉钉表

**2026-09-15 计划物流同事核实**：EN 销售订单上的 `po_no` / 行级 `purchase_order`
是**离职同事自编的内部流转号**，不是客户给的采购订单号。当年用途是「一张 SO 对应多个
客户 PO 时，区分下单日期 / 仓库」。现在 SO 自身已有下单日期和仓库，所以**意义有限、
之后可能弃用**。（同一天早先把它当「客户 PO 号 + 头程入口」写进文档，是错的，已更正。）

它仍有一个过渡期用途：同一 SO 下不同批次/仓库的区分。所以报表保留该列，
但列名写成 **「内部流转号(非客户PO,待废弃)」**，**不得**对外称客户 PO，**不得**拿它查头程。

**真实头程入口 = EN 销售订单编号 × 钉钉表**：

| 表 | 关键 sheet | join key |
|----|-----------|----------|
| 2026年下单表 | `2026年度订单明细` | **`EN销售订单编号`** == `Sales Order.name` |
| 2026年下单表 | `物流信息表` | 通途采购单号(`PO…`) → `ZMT…` 物流号 |
| 发货信息总表 | `物流跟踪Tracking` | 批次号(`NJHY…`/`TXHY…`/`PL…`) → 离港/到港/入库 |

`2026年度订单明细` 上有**工厂四件套**：`工厂确认交期` → `包装完成日期` →
`实际包装完成量` → `物流发票编号`。**逐级为空 = 生产完成但还没进头程**，
这是回答「为什么还没发」的直接证据。读取方式见 `dingtalk/dingtalk_sheet/`。

实测：`SO-26-00101` 本物料那行目的仓库 `美中仓`、下单量 40、需求交货 `2026/8/30`，
**四件套全空**；同表另一行备注直接写着 **`SO-26-00099作废`** ——
印证了下面 Why This Matters 里「疑似重复」的判断是对的，业务侧早已在表里作废。
...
- 要把「工厂出库以后去了哪」接上时：用 **EN 销售订单编号** 读钉钉
  `2026年度订单明细` / `发货信息总表`（见第 10 条）。**不要**用 EN 的 `po_no`（它不是客户 PO）。
