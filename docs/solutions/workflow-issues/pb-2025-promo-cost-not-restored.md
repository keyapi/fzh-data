...
会话在 Tracy 确认 Christine 改价表、Diane 尚未改主档时暂停。需要把口径、附件策略和三数据源边界写下来，避免下一任 Agent 再用 $28k 粗算或把财务表发票号发给 PB。
...
错误口径会把窗口内合法活动价、重复 CSV、或对方不认的发票号送进索赔，对不上 Diane 后台和 AP。佣金出现在 WSI 邮件里会破坏中间人关系。叠折会把下一轮活动做成 ~41 的批发价。

## When to Apply

- 本 case 从 Tracy 确认 Christine 表起继续。
- 任何「SPS 发票 vs PB remittance vs 已加工财务表」三方对票。
