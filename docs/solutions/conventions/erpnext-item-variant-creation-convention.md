...
- 有库存的**完整**通途 SKU（含 `-Cover`/`-Foam`）必须登记到至少一个 `KS` 产品成品变体的 `customer_items.ref_code`；去尾缀基码只能作为候选。
- `PK#`、`HM1510` 或 BOM 组件上的客户码不能替代产品成品登记；原因是 EN 销售订单 Excel 先用通途 SKU 找产品物料，再由交付形态列决定实际处理。
- 一个通途码已登记到多个产品变体时保留并报告；不在建物料时顺手清理。
- 完整主线流程见 [通途有库存 SKU 三方主线补齐惯例](tongtu-en-sellfox-instock-sku-mainline.md)。
