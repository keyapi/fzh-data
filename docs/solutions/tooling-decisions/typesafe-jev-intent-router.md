...
10 条标注中文样例 **10/10 命中**；「那个东西弄一下」与「帮我订一张去上海的机票」均正确落 `none`。单次约 5970 输入 token ≈ **$0.00025**。

两个反直觉结论，都已写进代码文档：

1. **`confidence` 几近饱和：0.97–1.00**（多数为 1.00，连 `none` 胜出时也有 0.99）。它是**分布集中度不是正确率**，所以 `--min-confidence` 实际**不触发** —— 它只防「模型犹豫」，不防「模型自信地分错」。**真正兜底的是 `none` 选项**。
2. **`ambiguity` 不具区分度**：只跨 0.87–0.98，清晰请求（stock-init，0.95）与域外请求（0.96）几乎重叠（方向正确但不是可用信号）。因此只在「无法判定」分支显示，命中分支不显示，避免把常数读成风险信号。
