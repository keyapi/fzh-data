...
4. href 集合退为**兜底**：旧行晚渲染（其提交时间 ≠ 新值）与小范围快任务仍被行锚定覆盖；
   仅当多轮锁不上（页面结构/解析异常）时才回退用「基线之外的新 href」，并打印告警。（2026-09 起）
...
行锚定上线后，实战又暴露/补齐了 4 点（代码见 `tongtu_orderdetail_report.py`，均已并入 main）：

1. **先读后切**：轮询改成**先读状态，未锁定/未完成才往返两 tab 刷新**（页面本身不自动刷新），
   减少无谓切换与一轮检测延迟。
2. **限流防护**：通途对报表生成有限流。提交后「等最上行变新」只给 **90s 窗口**
   （`LOCK_WAIT_SECS`）：窗口内没变新 → 视提交未生效/被限流，输出 **`RATE_LIMITED`**
   （页面出现「限额/频繁/排队/稍后再试」等字样）或 **`NO_NEW_JOB`** 退出，**不再无限往返切换**。
3. **复用已完成结果**：默认先看最上行是否已是「**今日**提交 + 条件含本次发货时间 + 已有下载链接」，
   是则**直接复用**、不再重复生成（`--no-reuse` 强制新生成）——既省时又避免触发限流。
4. **可观测性 / 启动统一**：脚本 stdout 改**行缓冲**（此前自建 `TextIOWrapper` 覆盖了 `PYTHONUNBUFFERED`，
   日志只在进程结束才落盘）；浏览器启动统一走 `legacy-compatible/browser_launch.py`
   （本机 bundled chromium 有头模式不可用时，用 `WEB_AUTOMATION_BROWSER_CHANNEL=chrome`）。

> 教训：**“等一个状态变化”的循环必须有上限**。没有上限时，只要那个状态因外部原因（限流/提交被拒）
> 永远不出现，循环就从“等待”退化成“死循环”；同时要**优先复用已完成的同范围结果**，避免重复触发外部配额。
