...
- **全量验证 + ops 表**: 1176 号全量跑（1148 GLS 包裹成功 + 39 停放/源数据非 GLS）；新增 `gls_track/ops_report.py` 生成 FedEx 风格 8-Sheet 异常表。口径与 FedEx 表不同：`HANDLING_DAYS=2`、营业日用波兰 2026 假日(非美国联邦)、周末不计、「Amazon是否判迟」仅 Amazon 渠道。输出 `通途非FBA订单202608 GLS运营异常表 20260907.xlsx`（1187 行：正常1058/在途39/查无39/承运延误30/漏发11/迟发9/卡件1）。
