...
**进度（2026-07-17）：** ②③④ 已收集并完成列分析 → [lizard-p0-column-mapping-2026-07-17.md](lizard-p0-column-mapping-2026-07-17.md)。匹配键为 `参考编号/Reference Code`（与 `packageSn` 同形、原样回传）；`物流单号`=追踪号。上传重量为克、返回为 kg。① 赛狐导出样例仅表头，待重导。现网路径仍是通途→小程序→蜴国际，① 与 ②–④ 非同批。
