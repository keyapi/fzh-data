...
- **调整单批次「跟不跟随备货单」取决于批次 `type`**（2026-09-18 更正，此前笼统写"会跟随"，不准确）：

  | type | 含义 | 批次行为 |
  |---|---|---|
  | **3** | 库存调整-**增加** | **新建独立批次**，成本是创建时该 (仓库,SKU) 均价的**快照** → **不跟随** |
  | **4** | 库存调整-**减少** | 引用已有批次，共享同一份成本 → 跟随 |
  | 5 | 海外仓备货单 | 自带批次 |

  早先那次观测（`OWS294A9T700024` 头程 4.08→7.77，派生批次 `20260529002385` 同步变）
  命中的是 **type=4**；实测 type=3 的批次**不跟随**（某 SKU 改备货单头程 8.12→4.08 后，
  3 张 `AD…增加` 批次仍停在 8.12）。
- 因此**「改成本只需要盯备货单」只对备货单批次和 type=4 成立**。type=3 批次
  **没有任何自动化入口可改**（调整单无成本字段），只能靠改用「其他入库单」避免再产生。
- 新增批次成本的取值规则（实测）：**= 创建时该 (仓库,SKU) 的加权平均成本快照**
  （库存明细的「采购单价 / 单位费用」），**不是**来源单的成本。

## 库存调整单：已完成 = 终局

- **删除被明确拒绝**：弹窗 `仅【待调整】、【待提交】状态的单据可删除`。
- **详情页没有撤销/作废**，只有 `取消`（= 返回列表）和 `打印`；端点族里没有 `antiAudit`/`revoke`。
- **`+N` 后用 `-N` 回不去**：扣减按 **FIFO 吃最老批次**，不会冲掉新建的批次
  （实测 +1/-1 后：备货单批次 139→138、新批次 1 件留下、单位费用净漂移 +0.0019 且清不掉）。
- 唯一能对已完成单做的写操作是 **`editRemark`**（body `{id, remark}`）。
- **UI 删除入口**：列表页勾选 → `更多`（`button.h_32`）→ `删除`。注意它是 **Element-UI hover 菜单**，
  Playwright 移动鼠标会把它关掉 —— 用 locator 点击而不是坐标点击。

## 备货单详情/编辑页的 `id` 从 localStorage 传，不从 URL

`stockOrder/editOrder|detailOrder/index.html` **忽略 URL 上的 `?id=`**，改用 localStorage：

```
DetailStockOrder_detail / EditStockOrder_edit = {"params":{"id":<pickId>}, "type":"detail"|"edit", "orderFrom":"..."}
```

先写 localStorage 再 `goto`，否则会打开上次查看的那张单（极易误判成"参数没生效"）。
（调整单的详情页 `adjustmentSheetNew/Detail.html?id=` **是**用 URL 传的，两者不同。）
...
> 更正：上面「两条都走不通」的说法**不成立**。后来验证 **`createV2` 单次调用即可建单并直接落库存**
> （不需要再调 `batchConfirmAdjust`；见 `docs/solutions/integration-issues/sellfox-adjust-order-write-chain.md`）。
> 我之前失败是因为**没有先查库存明细拿 `originId`**，以及走了内部网关那条路。
> 经代理返回 500 的原因仍未查清（可能是当时代理侧瞬时故障），但**公开 API 这条路是通的**。

### 调整单按 FIFO 扣批次（实测钉死）

2026-09-18 受控实验：`test001-white` @ POLAND 有两个在库批次各 1000，
建一张调整单 `AD2609180004` 扣 **-3**，结果：
...
→ **调整单（以及所有出库）按先进先出吃批次：先扣光最老的批次，不够再扣次老的。**
所以「10 张备货单各 1 件、调整单 -3」会扣**最老的 3 个批次各 1 件**，而不是平摊、也不是扣最新的。

**对成本工具的直接影响**：调整单/出库先吃**老批次**，等于把老批次那份成本从加权平均里拿走。
排激励价时必须把「老批次会被先扣走」一起算——否则改完新批次成本，一次出库就把均价带偏。
