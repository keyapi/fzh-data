...
- `KS0001` / `KS0248` 的 `PK# -> KS x1` 组合已在生产建完（2026-08-21，957 个）。创建走 `cover_combo_ops.py`，不要 `sync-combos`。
- 运营在赛狐按组合商品 / SKU `PK#` 查找，普通商品搜「皮壳#」会空。
- 写配对未授权：只出 `pairing-candidates`。库存扣减沙盒仍须用户点名仓+SKU。
...
5. `KS0001` / `KS0248` 组合已存在，不要全量再创建。其他 SPU 创建、在线配对、库存调整、测试下单必须再次确认。
...
- `audit_sandbox.py`：默认无写入；`--live` 只查仓库列表和商品 pageList（底层 + 皮壳 SKU），不查 Listing。
- `SELLFOX_API/cover_combo_ops.py`：`plan` / `status` / `apply` / `pairing-candidates`。手册 → [cover-combo-ops.md](../SELLFOX_API/docs/reference/cover-combo-ops.md)。
- `SELLFOX_API/cover_combo_plan.py`：无网络计划。
- `tests/sellfox_api/test_cover_combo_ops.py`：计划纯函数测试。
