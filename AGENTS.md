...
**① 先搜再造 (Search Before Building)** — 动手前先用模块索引定位已有实现。检查成本趋近于零，不检查的代价是重新发明一个更烂的轮子。参考 `docs/agent-guide.md` 了解现有 Skill。

**② 把湖煮干 (Boil the Lake)** — 数据管道不容"差不多"。SKU 匹配覆盖所有已知后缀，Excel 列验证全覆盖，边界条件写清楚。但不要煮海——CI/CD、监控面板、架构重写不是我们的湖。

**③ 用户主权 (User Sovereignty)** — Agent 推荐，用户决定。两个 Agent 达成一致≠可以动手。赛狐导入前必须确认范围，绝不擅自扩大到全量。你永远缺用户的领域上下文。
