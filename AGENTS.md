...
> [!NOTE]
> 本文件是项目的**唯一指令来源**，两边 Agent（Claude Code / Codex CLI）共用。
> `CLAUDE.md` 是此文件的 symlink，**不要直接编辑 `CLAUDE.md`**。
> 如果 `CLAUDE.md` 在你的系统上不是 symlink，请手动创建内容为
> `Read @AGENTS.md for more info` 的 `CLAUDE.md` 文件，
> 或启用 Windows Developer Mode + `git config core.symlinks true` 后重新 clone。
...
每个模块对应一个 Skill（存放于各 Agent 的 skill 目录），Agent 自动按用户触发词加载。详情见各 Skill 文件 + 模块的 `AGENT_HANDOFF.md`。
...
When debugging column changes, know which system produces each file. See Lesson #14 details in previous AGENTS.md versions or ask user.

### 15. Keep docs updated immediately
After every fix/discovery, write to AGENTS.md/md files in the same session. Don't wait for reminders.

### 16. Git worktree: commit from worktree, not main repo
Shell CWD resets to worktree but git commands could operate on main repo. Verify `git branch` shows the worktree branch before commit. If mistake: `git reset --hard <prev>` on main, `git cherry-pick <hash>` on worktree.
...
### 36. E2E 测试即断言验证
AGENTS.md 守则 4 "目标驱动执行"：改完代码就跑验证，不是打开浏览器看了算完。
...
The next agent session (whether yours, GQ's, or a colleague's) starts by reading AGENTS.md, SKILL.md, and AGENT_HANDOFF.md. If those files are stale, the agent will make decisions based on wrong information — wasting time and introducing bugs.
...
| **去重** | SKILL.md 不重复 AGENTS.md 内容，不重复 AGENT_HANDOFF.md 内容，用引用代替 |
