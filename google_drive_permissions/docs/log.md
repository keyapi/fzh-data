...
用户（Drive 属主账号下）有大量 Google 表 / Colab notebook 被历史共享给多名员工。存在：
1. 前财务负责人换成现任，需继承编辑权限；
2. 一批离职/停用账号仍残留权限；
...
- 给 12 张月度通途订单表（通途订单202501-202512）加现任财务负责人的 writer。
...
- 建立用户级 OAuth（Desktop OAuth client / Testing，scope drive）→ `gsheets-user-oauth.json`。
...
1. **4 个「属主是离职账号」的文件**：Mano DE / Mano FR / Vercart IT，无法靠清权限（Drive 不允许移除离职属主），需复制/重建。用户选择**暂不管**。
