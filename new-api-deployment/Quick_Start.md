...
```bash
# 从 .secrets.env 加载实际值（替换下方占位符）
source .secrets.env 2>/dev/null || echo "请手动配置 .secrets.env"
# 或直接读取文件中的变量值手动替换
```

> `.secrets.env` 包含: 管理员密码、DeepSeek API Key、Token Key 等

## 3. 管理 API 认证

```bash
# 登录获取 session cookie（密码从 .secrets.env 获取）
curl -s -c /tmp/na-cookies.txt -b /tmp/na-cookies.txt \
  -X POST http://localhost:3000/api/user/login \
  -H "Content-Type: application/json" \
  -d '{"username":"root","password":"<ADMIN_PASSWORD>"}' > /dev/null
...
```bash
# 使用管理员令牌（Token 值从 .secrets.env 获取）
...
## 9. 创建用户+令牌完整流程（新用户接入步骤）

```bash
# 1. 登录（密码从 .secrets.env 获取）
curl -s -c /tmp/cookies.txt -X POST http://localhost:3000/api/user/login \
  -H "Content-Type: application/json" \
  -d '{"username":"root","password":"<ADMIN_PASSWORD>"}'
...
| 项目 | 值（实际密码/Key 见 `.secrets.env`）|
|------|-----------------------------------|
| 管理后台 | http://localhost:3000 |
| 管理员 | root / 见 `.secrets.env` |
| DeepSeek API Key | 见 `.secrets.env` |
| 数据目录 | D:\docker\new-api\data\ |
| 日志目录 | D:\docker\new-api\logs\ |
| 数据库文件 | D:\docker\new-api\data\one-api.db |
| 定价同步脚本 | `sync_pricing.py` |
| 敏感信息文件 | `.secrets.env`（请勿提交）|
