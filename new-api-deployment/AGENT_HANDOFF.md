...
| 用户 | 角色 | 密码 | 用途 |
|------|------|------|------|
| `root` | RootUser (100) | `admin123456` | 管理员 |
| `limittest` | CommonUser (1) | — | 额度测试用户 |
| `newuser` | CommonUser (1) | `new123456` | 普通用户 |

### 渠道配置

| 名称 | 类型(代码) | 模型 | 上游地址 | API Key |
|------|-----------|------|---------|---------|
| DeepSeek Test | DeepSeek (43) | `deepseek-v4-flash,deepseek-v4-pro` | `https://api.deepseek.com` | `sk-8832c70267b14492a7e5afdbf8e3f953` |

### 当前活跃令牌

| Token Key | 所属用户 | 额度 | 特性 |
|-----------|---------|------|------|
| `BTJlbnLTgvsSJbQvPuWv3ruKCYgdw4AJkLlUWfbotniXdvux` | limittest | 6849 | 5分钟重置套餐绑定 |
| `HiaKpzR410JQiMh73YwlYpgjGrE66vXjgR1dECgl0A8KPznC` | root | 无限 | 管理员测试用 |

### Web 后台

> http://localhost:3000 | 管理员: root / admin123456
...
- **恢复**: 从备份文件 `/tmp/ratio_output.txt` 恢复，用 docker cp + readfile() 写回
...
### 同步脚本
`D:\Claude Demo\fzh-data\new-api-deployment\sync_pricing.py` — 修改顶部定价数字后运行
