...
后来 catalog 增到 34 → 46 → 56 项各重跑一次、标注样例同步扩到 31 条，均全中，`confidence` 0.98–1.00）：
...
**结论一：`confidence` 在本目录上 0.98–1.00、几近饱和**，连 `none` 胜出时也是。它是**分布集中度**不是
...
单次成本：随 catalog 变大而增 —— 33 项约 5550、56 项约 **9815 输入 token ≈ $0.00041**（$0.042/M 输入）。
